...
const statuses: {[key: string]: string} = { Completed: 'text-green-400 bg-green-400/10', Error: 'text-rose-400 bg-rose-400/10' }
const activityItems = [
  {
    user: {
      name: 'Michael Foster',
  ...
  return (
    <div className="bg-white py-10 w-full">
        <div className="mx-auto max-w-2xl lg:text-center">
            <h2 className="text-base font-semibold leading-7 text-indigo-400">Join now</h2>
                ...
                {activityItems.map((item) => (
                    <tr key={item.commit}>
                        ...
                        <td className="hidden py-4 pl-0 pr-4 sm:table-cell sm:pr-8">
                            ...
                            <div className="text-sm leading-6 text-gray-900">{item.commit}</div>
                            <div className="rounded-md bg-gray-300/40 px-2 py-1 text-xs font-medium text-gray-400 ring-1 ring-inset ring-black/10">
                                {item.branch}
                            </div>
                            </div>
                        </td>
                        <td className="hidden py-4 pl-0 pr-8 text-sm leading-6 text-black-400 md:table-cell lg:pr-20">
                            {item.duration}
                        </td>
                        <td className="hidden py-4 pl-0 pr-4 text-right text-sm leading-6 text-black-400 sm:table-cell sm:pr-6 lg:pr-8">
                            <time dateTime={item.date.toDateString()}>{item.date.toDateString()}</time>
                        </td>
                    </tr>
                ))}
            </tbody>
